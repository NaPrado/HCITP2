...
export async function createUser(newUser: User, mailer: Mailer): Promise<User> {
  try {
    ...
    if (
      (err as QueryFailedError).driverError.message.includes(
...
export async function resendUserVerification(
  ...
  try {
    ...
    // Primero eliminamos todos los códigos de verificación anteriores
    const oldVerifications = user.verification.filter(
      (v) => v.type == VerificationType.REGISTRATION
    );
    if (oldVerifications.length > 0) {
      await Promise.all(oldVerifications.map((v) => v.remove()));
    }

    // Luego creamos el nuevo código
...
export async function verifyUser(code: string): Promise<UserData> {
  try {
    const verification: Verification = await VerficationService.getVerification(
      VerificationType.REGISTRATION,
      code
    );

    if (verification.expirationDate < new Date())
      throw new BadRequestError("Expired code.");
...
export async function resetUserPassword(
  ...
  try {
    ...
    const oldVerification = user.verification.find(
      (v) => v.type == VerificationType.RESET_PASSWORD
    );
    ...
    mailer.sendEmail(
      ...
      newVerification.code,
      newVerification.expirationDate
    );
  } catch (err: unknown) {
    throw err;
  }
