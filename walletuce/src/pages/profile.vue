            ...
            <v-container class="d-flex flex-column align-center justify-center main-content pt-4">
                <v-card class="pa-6 container-card bg-grey-lighten-2" rounded="lg">    
                    <v-card class="pa-4 mb-4 card-with-shadow-medium bg-grey-lighten-3" rounded="lg">
                        <v-card-title class="text-green font-weight-bold pb-1 pt-0"><span><h2>{{nombre}}</h2></span></v-card-title>
                        <v-card-title class="text-black pb-0 pt-0"><span><h4>ID de cuenta: {{id_cuenta}}</h4></span></v-card-title>
                        <v-card-title class="text-black pb-0 pt-0"><span><h5>Mail asociado: {{mail}}</h5></span></v-card-title>
                    ...
                    <v-row class="mt-4 mb-1 mr-1" justify="end" align="center">
                        <v-btn color="green-lighten-1" class="text-white font-weight-bold pl-6 pr-6 ml-3" rounded @click="onCerrarClick">
                            Cerrar Sesión
                        </v-btn>
                    </v-row>
                </v-card>
...
const mail = ref("matipalermo@gmail.com");
const nombre = ref("Matias Palermo");
const id_cuenta = ref(12344321);
